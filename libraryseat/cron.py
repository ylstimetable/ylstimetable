...
def schedule_every_ten_minutes():
    result_count = len(Result.objects.filter(semester='2023-2'))

    if result_count != 0:
        result = Result.objects.filter(semester='2023-2')
        for a in result:
            result = a
        receipt = Receipt.objects.filter(semester='2023-2')
        for a in receipt:
            receipt = a
        ...
        random_result = Result.objects.filter(semester='2023-2')
