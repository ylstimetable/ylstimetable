...
def receive(request):
    ...
    q = Receipt.objects.filter(semester='2026-1')
    for obj in q:
        q_obj = obj
...
def random(request):
    ...
    applicant_list = Receipt.objects.filter(semester='2026-1')
    ...
    strings = ','.join(target)
    a = Result(semester='2026-1', sequence=strings, sequence_backup=strings)
    a.save()
...
def register(request):
    result = Result.objects.filter(semester='2026-1')
    for re in result:
        result = re
...
def index(request):
    result = Result.objects.filter(semester='2026-1')
    receipt_start = len(Receipt.objects.filter(semester='2026-1'))
    location = 10000
    random_start = 0
    user = request.user
    ...
    if len(Result.objects.filter(semester='2026-1')) == 0:
...
def seat_register(request, seat_number):
    ...
    smoking_zone = [81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 1139, 1140, 1141, 1142, 1143, 1144, 1161, 1162, 10076, 10077, 10078, 10081, 10082, 10083, 10086, 10087, 10088]

    requested_seat = int(seat_number)
    current_queue = Result.objects.filter(semester='2026-1')
